...
--[[
	local AES = require(11195079384)

	local encrypted = AES.encrypt_CTR("PASSWORD", "TEXT")
	local decrypted = AES.decrypt_CTR("PASSWORD", encrypted)
	print(string.char(unpack(decrypted)))
]]
...
local function init(key, txt, m, iv, s)
	key = convertType(key)
